...
| Claude Desktop Cowork | `imweb-ai-toolkit.plugin`, `imweb.skill`, `.claude-plugin/plugin.json`, `.mcp.json`, `bin/imweb-mcp.mjs` package | `.claude-plugin/marketplace.json` | Cowork task가 `.plugin`/`.skill` package를 생성/검증하고 host에 install artifact로 제시. 설치 후 `/imweb 주문목록을 확인해줘` 또는 자연어 요청이 local MCP bridge를 통해 host `imweb` CLI/auth를 사용. Claude Code registry/`~/.claude/skills`와 별개 | 지원 |
