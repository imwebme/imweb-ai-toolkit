...
이 명령은 public GitHub 저장소를 지속 가능한 marketplace source로 등록하고, Claude Code plugin을 설치하며, Codex가 바로 탐색할 수 있도록 `imweb` skill을 복사합니다. Claude Desktop Cowork용으로는 agent에게 `--tool claude-desktop`으로 `imweb-ai-toolkit-plugin.zip`을 만들게 한 뒤 Cowork Plugins UI에서 업로드합니다. agent용 설치와 검증 checklist는 [docs/ai-agent-installation.md](docs/ai-agent-installation.md)를 봅니다.
...
Codex는 marketplace 등록 후 Plugins UI에서 설치합니다. Claude Code는 등록된 marketplace에서 바로 설치하고 `/imweb-ai-toolkit:imweb`로 plugin skill을 검증할 수 있습니다. Claude Desktop Cowork는 별도 Desktop plugin 설치 경로를 사용하므로 생성한 plugin zip을 업로드하거나 조직 marketplace를 사용합니다.
