...
이 경로는 `npx` 임시 package 디렉터리가 아니라 public Git repository를 marketplace source로 등록합니다. Cowork plugin package는 agent가 명령을 실행한 디렉터리에 `imweb-ai-toolkit-plugin.zip`로 생성하고, 직접 `/imweb`용 Skill package는 `imweb-skill.zip`로 생성합니다. Claude에게 설치를 맡기는 요청문은 [../docs/cowork-ask-claude-install.md](../docs/cowork-ask-claude-install.md), 자세한 agent용 절차는 [../docs/ai-agent-installation.md](../docs/ai-agent-installation.md)를 봅니다.
...
- Claude Desktop Cowork plugin: `--package` 또는 `-Package`로 custom plugin upload용 zip을 생성합니다.
- Claude Cowork direct `/imweb`: `--skill-package` 또는 `-SkillPackage`로 custom Skill upload용 zip을 생성합니다. Desktop UI에서 Cowork > Customize > Skills에 올리면 `/imweb`가 직접 entrypoint가 됩니다. Claude Code CLI registry와 `~/.claude/skills`는 Desktop Cowork 설치를 대체하지 않습니다.
