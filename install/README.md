...
Claude Cowork에서 직접 `/imweb`가 필요하면 custom Skill package도 생성합니다.

```bash
npx --yes github:imwebme/imweb-ai-toolkit --tool claude-cowork
```

이 경로는 `npx` 임시 package 디렉터리가 아니라 public Git repository를 marketplace source로 등록합니다. Cowork plugin package는 agent가 명령을 실행한 디렉터리에 `imweb-ai-toolkit-plugin.zip`로 생성하고, 직접 `/imweb`용 Skill package는 `imweb-skill.zip`로 생성합니다. Cowork task 안에서는 computer-use나 Claude Desktop UI 조작으로 설치를 시도하지 않습니다. Claude에게 package 생성과 검증을 맡기는 요청문은 [../docs/cowork-ask-claude-install.md](../docs/cowork-ask-claude-install.md), 자세한 agent용 절차는 [../docs/ai-agent-installation.md](../docs/ai-agent-installation.md)를 봅니다.
...
- Claude Desktop Cowork plugin: `--package` 또는 `-Package`로 plugin/조직 provisioning용 zip을 생성합니다.
- Claude Cowork direct `/imweb`: `--skill-package` 또는 `-SkillPackage`로 custom Skill provisioning용 zip을 생성합니다. Cowork task가 현재 세션에서 새 Skill을 로드할 수 없으면, 검증된 zip을 사용자, workspace admin, 또는 지원되는 Cowork Skill 설치 흐름에서 나중에 provisioning해야 합니다. Claude Code CLI registry와 `~/.claude/skills`는 Desktop Cowork 설치를 대체하지 않습니다.
