...
이 경로는 `npx` 임시 package 디렉터리가 아니라 public Git repository를 marketplace source로 등록합니다. Cowork plugin package는 agent가 명령을 실행한 디렉터리에 `imweb-ai-toolkit.plugin`으로 생성하고, fallback Skill package는 `imweb-skill.zip`로 생성합니다. Package에는 host `imweb` CLI/auth를 쓰는 local MCP bridge가 포함됩니다. Cowork task 안에서는 computer-use나 Claude Desktop UI 조작으로 설치를 시도하지 않습니다. Claude에게 package 생성, 검증, `.plugin` artifact 제시를 맡기는 요청문은 [../docs/cowork-ask-claude-install.md](../docs/cowork-ask-claude-install.md), 자세한 agent용 절차는 [../docs/ai-agent-installation.md](../docs/ai-agent-installation.md)를 봅니다.
...
- Claude Desktop Cowork plugin: `--package` 또는 `-Package`로 설치 가능한 `.plugin` package를 생성합니다. 이 package에는 local MCP bridge와 skill/command가 함께 들어갑니다.
- Claude Cowork fallback Skill: `--skill-package` 또는 `-SkillPackage`로 custom Skill package zip을 생성합니다. 기본 경로는 `.plugin` artifact를 Cowork에 제시해 host가 설치/활성화하도록 하는 것입니다. Claude Code CLI registry와 `~/.claude/skills`는 Desktop Cowork 설치를 대체하지 않습니다.
